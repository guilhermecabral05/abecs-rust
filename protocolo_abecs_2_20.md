...
##### Versão: 2.20 (28-fev-25)

```
Copyright 2013-2025 © Abecs
...
```
```
2.20 28/fev/25 WFM (SETIS) ▪ Quantidade mínima de chaves aumentada para 64^ [^01 ][^04 ].
▪ Corrigido tamanho de CHP_CMD no comando “CHP”^ [^02 ].
▪ Revisada observação dos campos GOC_CONNECT^ [^03 ] e
SPE_GOXOPT^ [^04 ].
```
```
2.20 20/dez/24 WFM (SETIS) ▪ Incorporação das Atualizações de Especificação SU001 [^01 ][^02 ][^03 ]^
[ 04 ][ 05 ][ 06 ][ 07 ][ 08 ][ 09 ][ 10 ], SU002 [ 11 ], SU004 [ 12 ][ 13 ], SU005 [ 14 ][ 15 ][ 16 ] e
SU006 [^17 ][^18 ].
▪ Incluída referência ao kernel EMVCo C-8 (uso futuro)^ [^19 ].
▪ Comando “CHP” retorna ST_INVPARM caso não suporte o
SAM referido [^21 ][^22 ][^23 ].
▪ Revisados tempos de espera no protocolo de comunicação
serial [^25 ][^26 ].
▪ O parâmetro T1_CTLSAPPVER da Tabela de AID não tem mais
uso e foi obsoletado^ [^28 ].
▪ Melhorado detalhamento do TTQ Visa [^29 ] e Discover [^30 ].
▪ Incluídas em PP_CAPAB (na resposta de “GIX”) informações de
suporte a cartão magnético, ICC, SAM e ausência de display [^31 ].
▪ Incluído suporte a cartões Quick Pass (Union Pay sem contato)^
[ 32 ][ 33 ][ 34 ][ 35 ][ 36 ].
```
```
▪ Incluído PP_CTLSCAPAB, PP_BATTINFO e PP_COMMINFO na
resposta do comando “GIX”^ [^37 ][^45 ][^46 ][^47 ].
▪ Revisados retornos de erro de CTLS em “GCR” e “GCX”^
[ 48 ][ 50 ][ 51 ][ 52 ][ 53 ].
```
```
▪ Objeto Track 1 Data ( tag 56h) é dado protegido^ [^54 ].
▪ O comando “GCR” também mostra o valor da transação ao
solicitar o cartão^ [^57 ].
▪ Melhoria na descrição de quais campos são retornados na
resposta ao “GCR” para cada tipo de cartão [^56 ].
▪ Incluído parâmetro no comando “DEX” para com opções para o
aprimoramento da apresentação das mensagens^ [^62 ][^63 ].
▪ O parâmetro T1_MOBCVM da Tabela de AID passa a ser
ignorado pelo pinpad, ficando mandatório o suporte à verificação
de portador pelo dispositivo móvel [^64 ][^65 ][^66 ][^67 ][^68 ][^69 ].
▪ Incluído suporte a APDU estendido no comando “CHP”^ [^70 ].
▪ Excluídos todos os comandos genéricos “GEN”^ [^71 ].
▪ Incluída entrada alfanumérica no comando “GCD”^ [^72 ][^73 ][^74 ].
▪ Outros ajustes e explanações sem mudança funcional [^20 ][^24 ][^27 ]
[ 38 ][ 39 ][ 40 ][ 41 ][ 42 ][ 43 ][ 44 ][ 49 ][ 55 ][ 58 ][ 59 ][ 60 ][ 61 ][ 75 ].
```
```
...
▪ Excluídos métodos para carga remota de chaves e os comandos
“IKS” e “IKE”.
...
```
```
2.10 04/abr/18 WFM (SETIS) ▪ Incorporação das Atualizações de Especificação SU001 [^01 ],
SU002 [^02 ][^03 ], SU003 [^04 ][^05 ][^06 ][^07 ], SU004 [^08 ][^09 ] e SU005 [^10 ].
▪ Inclusão das mensagens de 000Dh a 0035h no comando “GCD”
[ 11 ].
```
```
...
- 2. Protocolo de Comunicação
   ...
   - 2.2. Nível de Enlace
      ...
      - 2.2.2. Fluxo de comunicação
         ...
         - 2.2.2.2. Devolução de resposta pelo pinpad
...
Independent ICC to Terminal Interface Requirements - Version 4. 4 - October 2022_.

 **EMV#2** EMV - _Integrated Circuit Card Specifications for Payment Systems - Book 2 - Security and
Key Management - Version 4.4 - October 2022_.

 **EMV#3** EMV - _Integrated Circuit Card Specifications for Payment Systems - Book 3 - Application
Specification - Version 4.4 - October 2022_.

 **EMV#4** EMV - _Integrated Circuit Card Specifications for Payment Systems - Book 4 - Cardholder,
Attendant, and Acquirer Interface Requirements - Version 4.4 - October 2022_.

 **EMV#CtlsA** EMV - _Contactless Specifications for Payment Systems - Book A - Architecture and
General Requirements - Version 2.11 - June 2023_.

 **EMV#CtlsB** EMV - _Contactless Specifications for Payment Systems - Book B - Entry Point Specification_

_- Version 2.11 - June 2023_.
...
 **QckPass** EMV - _Contactless Specifications for Payment Systems - Book C- 7 - Kernel 7 Specification_
...
**APDU** Ou “ _Application Protocol Data Unit_ ”, trata-se do formato padrão de uma mensagem de
comando enviada pelo pinpad ao ICC ( **C-APDU** ) ou de uma mensagem de resposta
devolvida ao pinpad pelo ICC ( **R-APDU** ).
...
**Bypass** Situação na qual o portador do cartão se recusa a digitar o PIN, pressionando a tecla
[OK/ENTRA] do pinpad com o campo de entrada vazio.
...
quaisquer) definido pela norma ANSI X9.24:2009.
...
**MK** Ou “ _Master Key_ ”, chave de criptografia TDES inserida no pinpad (nesta especificação
referenciada como **MK:TDES** ).
...
**Trilha** Um dos três blocos de dados gravados em um cartão magnético, referidos como Trilha
1, Trilha 2 e Trilha 3. Esses blocos de dados geralmente contêm o PAN, a data de


```
validade e outras informações relevantes. O ICC e o CLTS podem conter os mesmos
blocos de dados em sua memória.
```
...
```
destaque ) de maneira a se evitar a perda de referência e, assim, facilitar sua
compreensão.
```
...
mensagens de notificação), conforme formato descrito na seção 2.2.1.
```
```
SPE Pinpad
```
...
resposta coerente ( RSP_ID com o mesmo valor de CMD_ID ). Neste caso, utiliza-se a seguinte
resposta:
```
...
```
```
ST_CARDBLOCKED 079 Cartão ICC ou CTLS está bloqueado.
```
```
...
```
```
ST_CTLSINVALIDAT 082 CTLS foi invalidado (reservado).
```
```
...
parâmetro, por exemplo). Se a situação exigir o parâmetro e o SPE não o enviar, o pinpad
retornará ST_MANDAT.
```
```
O = Parâmetro é opcional e o SPE somente o enviará caso desejado para o processamento do
comando. Caso o pinpad necessite desta informação, ele deve usar um valor “default”
...
“1” = MK/WK:TDES:PIN; e
“3” = DUKPT:TDES:PIN (ver seção 5.1.1 ).
```
```
...
**SPE_WKENC** 000Ah B16 _Working Key_ criptografada pela MK:TDES.
...
em segundos (0 a 255), sendo que 0 indica que o
comando retorna imediatamente.
...
**SPE_DSPMSG** 001Bh S..128 Mensagem a ser apresentada no _display_ do pinpad, em
formato livre, com caracteres de quebras de linha (0Dh).
Ao formatar esta mensagem, o SPE deve se atentar às
capacidades do _display_ do pinpad (ver **PP_DSPTXTSZ** ).
...
**SPE_GCDOPT** 0026h N4 Opções do comando “ **GCD** ”:
“ **0** xxx” = Entrada numérica;
“ **1** xxx” = Entrada alfanumérica;
“x000” = RUF.
...
deverá finalizar a operação com erro fatal.
```
```
MD Dado é mandatório dependendo da situação (pode ser um complemento de outro dado, por
exemplo). Se a situação exigir um dado não devolvido pelo pinpad, o SPE deverá finalizar a
operação com erro fatal.
```
```
MR Dado é mandatório se requerido pelo SPE no comando recebido.
O = Dado é opcional e depende do processamento. Sua eventual ausência não deve ser criticada
pelo SPE.
```
...
“x 0 xxxxxxxx” = Não Possui display não gráfico;
“x 1 xxxxxxxx” = Possui display gráfico monocromático;
“x 2 xxxxxxxx” = Possui display gráfico colorido.
“x 9 xxxxxxxx” = Não possui display.
“xx 0 xxxxxxx” = O suporte a ICC não é informado
(especificação ≤ V2.12);
“xx 1 xxxxxxx” = Suporta ICC;
“xx 9 xxxxxxx” = Não suporta ICC.
“xxx 0 xxxxxx” = O suporte a cartão magnético não
é informado (especificação ≤ V2.12).
“xxx 1 xxxxxx” = Suporta cartão magnético trilhas 1, 2 e 3.
“xxx 2 xxxxxx” = Suporta cartão magnético trilhas 1 e 2.
“xxx 3 xxxxxx” = Suporta cartão magnético trilha 1.
“xxx 4 xxxxxx” = Suporta cartão magnético trilha 2.
“xxx 9 xxxxxx” = Não suporta cartão magnético.
“xxxx 0 xxxxx” = O suporte a SAM não é informado
(especificação ≤ V2.12).
“xxxx N xxxxx” = Possui slots de SAM de 1 a “N”.
“xxxx 9 xxxxx” = Não possui slot para SAM.
“xxxxx00000” = RUF.
```
**PP_SOVER**^ **(†)** 8006 h A..20 Versão do _software_ básico ou sistema operacional
(formato livre).
...
**PP_CTLSCAPAB**^ **(†)** 800Bh A..16 Lista contendo os padrões de CTLS suportados, com
codificação similar à usada em **T1_CTLSMODE**. Para esta
especificação, o valor a ser retornado é “ **2469B** ”, sendo:
“2” = VISA payWave;
“4” = Mastercard PayPass;
“6” = Amex Expresspay;
“9” = Discover D-PAS; e
“B” = Union Pay Quick Pass.

**PP_KRNLVER**^ **(†)** 8010h A..20 Versão do Kernel EMV para ICC.

**PP_CTLSVER**^ **(†)** 8011h A..20 Versão principal (gerenciador ou _Entry Point_ ) dos Kernels
EMV para CTLS.
...
**PP_AECTLSVER**^ **(†)** 8014h A..20 Versão do Kernel EMV CTLS para cartões Amex
ExpressPay.

**PP_DPCTLSVER**^ **(†)** 8015h A..20 Versão do Kernel EMV CTLS para cartões Discover D-PAS.
...
```
```
Versão do Kernel EMV CTLS para cartões Pure.
Reservado.
```
**------** 8017h --- Reservado para JCB.

**PP_QPCTLSVER**^ **(†)** 8018 h A..20 Versão do Kernel EMV CTLS para cartões Union Pay Quick
Pass.
...
**PP_MKTDESP**^ **(†)** 8032h A100 100 caracteres contendo o mapa de chaves **MK:TDES:PIN**
...
“00” = Cartão;
“01”, “07” , “08”, “09”, “14”, “16”, “21” = Telefone
móvel;
“02” = Chaveiro;
“03”, “18”, “19”, “23” = Relógio;
“04”, “27” = Etiqueta móvel (“ _mobile tag_ ”);
“05” = Pulseira / bracelete;
“06” = Capa de telefone móvel (“ _case/sleeve_ ”);
“10”, “11”, “12”, “13”, “15”, “17”, “22” = _Tablet_ ou _e-
Reader_ ;
Outros valores = Uso futuro.
------ 8061h --- Reservado
**PP_TLRMEM (†)** 8062h X4 Quantidade de memória disponível (em bytes) para carga
...
**PP_BATTINFO** 8064h B2 Informações sobre bateria somente para pinpads móveis,
no formato XXh YYh, sendo:
XXh (X1) = Nível da bateria de 0% (00h) a 100% (64h); e
YYh (B1) = Máscara de bits, sendo:
‘xxxxxxx0’ = se desconectado da fonte de carga;
‘xxxxxxx1’ = se conectado à fonte de carga;
‘0000000x’ = RUF
**PP_COMMINFO** 8065h N4 Informações sobro o modo de comunicação sendo usado
pelo pinpad no momento:
“0xxx” = Serial RS-232 ou USB;
“1xxx” = Bluetooth;
“2xxx” = Wi-Fi;
“7xxx” = Outro/proprietário (com fio);
“8xxx” = Outro/proprietário (sem fio);
“9xxx” = Biblioteca / software.
“x000” = RUF
------ 9000h
a
9063h
...
31 35 36 41 41 31 41 30 41 35 35 45 32 46 31 41
...
44 39 42 43 32 45 30 41 45 44 37 35 41 44 37 34
...
```
```
PP_CTLSCAPAB(†) MR Somente se o pinpad suportar cartões sem contato.
```
```
PP_KRNLVER(†) MR
```
```
PP_CTLSVER(†) MD Somente se o pinpad suportar cartões sem contato.
```
```
PP_MCTLSVER(†) MD Somente se o pinpad suportar cartões sem contato.
```
```
 Obsoleto
 Blocante
...
```
```
PP_VCTLSVER(†) MD Somente se o pinpad suportar cartões sem contato.
```
```
PP_AECTLSVER(†) MD Somente se o pinpad suportar cartões sem contato.
```
```
PP_DPCTLSVER(†) MD Somente se o pinpad suportar cartões sem contato.
```
```
PP_PUREVER(†) MD Somente se o pinpad suportar cartões sem contato.
```
```
PP_QPCTLSVER(†) MD Somente se o pinpad suportar cartões sem contato.
```
```
PP_DSPTXTSZ(†) MR
```
...
```
```
PP_BATTINFO MD Somente para pinpads móveis com bateria.
PP_COMMINFO MR
```
```
...
PP_SOVER, PP_SPECVER, PP_MANVERS, PP_APPVERS, PP_GENVERS, PP_CTLSCAPAB, PP_KRNLVER,
PP_CTLSVER, PP_MCTLSVER, PP_VCTLSVER, PP_AECTLSVER, PP_DPCTLSVER, PP_PUREVER,
PP_QPCTLSVER, PP_DSPTXTSZ, PP_MKTDESP, PP_MKTDESD, PP_DKPTTDESP, PP_DKPTTDESD e
PP_TLRMEM.
...
```
47 49 58 30 30 30 36 3 6 34 80 01 00 08 30 31 39
...
20 80 0B 00 05 32 34 36 39 42 80 10 00 06 33 2E
30 30 36 41 80 11 00 04 31 2E 35 34 80 12 00 06
4D 43 31 2E 30 38 80 13 00 06 56 53 31 2E 30 32
80 14 00 06 41 45 31 2E 30 35 80 15 00 07 44 43
31 2E 30 30 42 80 1 8 00 08 51 50 55 50 56 34 30
32 80 20 00 04 30 34 32 30 80 32 00 64 30 30 31
30 30 30 30 30 30 30 30 31 30 30 30 30 30 30 31
30 30 30 30 31 30 30 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 80 33 00 64 30 30 31 30 30 30 30 30 30 30 30
30 30 30 30 30 30 30 31 30 30 30 30 30 30 30 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 80 35 00 64 30 31 31
31 30 31 30 30 30 31 31 31 31 30 30 31 31 30 30
31 30 30 30 30 30 31 30 30 30 30 30 30 30 30 30
30 32 32 32 32 32 32 32 32 32 32 32 32 32 32 3 2
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 80 36 00 64 30 30 31 30 30 30 30 30 30 31 30
31 30 30 30 31 30 30 30 31 30 30 30 30 30 31 30
30 30 30 30 30 30 31 30 30 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 32 32 32 32 32 32 32
32 32 32 32 32 32 32 32 32 80 62 00 04 00 01 86
A0
```
```
GIX000 664 €...019
...
```
```
DWK_WKPAN H32 WKPAN criptografada pela MK.
```
...
42 33 31 36 31 43 42 34 38 37 39 34 36 34 45 42
...
```
```
Adicionalmente, este comando possibilita a captura de um PIN para verificação direta no cartão, seja
qual for a tecnologia (ICC, SAM ou CTLS).
```
#####  Comando
...
CHP_CMD H.. 497 Comando a ser enviado ao cartão.
Se CHP_OPER = “2”, aceitam-se seguintes formatos de C-APDU:
CLA INS P1 P2
CLA INS P1 P2 Le
CLA INS P1 P2 Lc XX XX ... XX
CLA INS P1 P2 Lc XX XX ... XX Le
Para CTLS, ainda se pode usar “C-APDU estendido”, nos
seguintes formatos:
CLA INS P1 P2 00 Lc+ Lc- XX XX ... XX
CLA INS P1 P2 00 Lc+ Lc- XX XX ... XX Le
...
(não se aplica a CTLS).
ST_ERRCARD ............... Erro de comunicação entre o pinpad e o
ICC.
ST_CTLSMULTIPLE ...... Mais de um CTLS foi apresentado ao leitor
simultaneamente.
ST_CTLSCOMMERR ..... Erro de comunicação entre o pinpad
(antena) e o CTLS.
...
O campo DEX_OPTIONS é opcional e, caso presente, o pinpad usa as informações para gerar uma
apresentação mais direcionada usando ícones, cores e som, de forma a melhorar a acessibilidade. A
maneira como o pinpad monta a tela fica a cargo do fabricante, não sendo objeto desta
especificação.
```
#####  Comando
...
```
```
DEX_OPTIONS N6 Parâmetros para apresentação da mensagem:
Alinhamento horizontal:
“0xxxxx” = À esquerda (default);
“1xxxxx” = À direita;
“2xxxxx” = Centralizada.
Alinhamento vertical:
“x0xxxx” = Superior (default);
“x1xxxx” = Inferior;
“x2xxxx” = Centralizada.
Tipo de mensagem:
“xx0xxx” = Genérica (default);
“xx1xxx” = Sucesso;
“xx2xxx” = Erro;
“xx3xxx” = Alerta;
“xx4xxx” = Informativa.
“xxx000” = RUF.
Este parâmetro é opcional. Se não fornecido, o pinpad assume os
valores marcados como “default”.
```
```
 Obsoleto
 Blocante
...
```
```
SPE envia mensagem de erro centralizada na parte superior do display.
```
###### SPE 

```
44 45 58 30 34 30 30 33 31 4E 41 4F 20 41 55 54
4F 52 49 5A 41 44 41 0D 54 45 4E 54 45 20 4E 4F
56 41 4D 45 4E 54 45 21 32 30 32 30 30 30
```
```
DEX040031NAO∙AUT
ORIZADA.TENTE∙NO
...
```
Este comando envia uma mensagem ao display do pinpad em um formato fixo de
...
CRIPTOGRAFADO”, usando a chave DUKPT:TDES de índice “07”, com variante # 3 e modo CBC.
```
###### SPE 
...
44 4F 00 03 00 02 35 31 00 09 00 02 30 37
```
```
EBX0 40 ....DADO∙A
∙SER∙CRIPTOGRAFA
...
```
```
ENB_WKENC H32 Working Key criptografada pela MK.
```
```
...
 **PP** 45 4E 42 30 30 30 30 31 36 46 43 31 43 37 41 41 43 38 35 32 45 35 44 39 46 ENB000016FC1C7AAC852E5D9F
...
apresentada no display pinpad para a solicitação dos dados deve ser escolhida entre uma das
disponíveis em uma tabela fixa definida por esta especificação.
```
#####  Comando
...
```
```
SPE_GCDOPT O Opções do comando “ GCD ”:
“ 0 xxx ” = Entrada numérica;
“ 1 xxx ” = Entrada alfanumérica;
“x000” = RUF.
```
...
```
47 43 44 30 30 30 30 31 33 80 4D 00 09 31 36 39
39 33 37 38 32 33
```
```
GCD000013€M..169
...
```
```
GDU_IDX N2 Índice do registro de tratamento DUKPT:TDES:PIN.
```
#####  Resposta
...
retorna teclas numéricas, sendo que o pressionamento destas teclas é simplesmente ignorado pelo
pinpad durante a execução do comando.
```
```
...
dados criptografados segundo os métodos MK/WK:TDES ou DUKPT:TDES.
```
```
...
Se GPN_METHOD = “3”, este campo é ignorado pelo pinpad.
```
```
GPN_PANLEN N2 Tamanho do PAN (de “02” a “19”).
Caso este comando esteja sendo usado depois de “ GCX ” ou “ CEX ”,
Se a criptografia “ End-to-End ” estiver sendo utilizada (ver seção
5.4 ) e o comando “ GTK ” ainda não foi executado, deve-se pode-se
fornecer um PAN “vazio” (tamanho “00”) para que o pinpad
considere o PAN já armazenado em sua memória, preservando-se
assim a segurança da criptografia “ End-to-End ” (ver seção 5.4 ).
```
```
GPN_PAN A19 PAN, alinhado à esquerda (espaços à direita).
Se o pinpad estiver em modo “PAN Criptografado”, este campo
deve ser codificado usando-se TDES-^1 ( reverso ) com a chave WKPAN
(ver seção 5.3 ).
```
```
...
```
```
GPN_PINBLK H16 Resultado criptografado.
```
```
GPN_KSN H20 Número de série da chave ( Key Serial Number ), somente no caso
de DUKPT ( GPN_METHOD = “3”). Para MK/WK, este campo é
devolvido zerado.
```
#####  Exemplos

```
SPE solicita a captura de PIN usando o método MK/WK:TDES, com chave de índice “08”.
```
###### SPE 
...
PAN). Os dados podem ser devolvidos “em claro” ou criptografados conforme definido na seção 5.4.
```
```
...
```
```
SPE  47 54 4B 30 32 3 30 31 31 31 00 08 00 01 36 00 09 00 02 31 32^5 00 03 00 02 34 30 00 07 00 0 4 GTK02 0 111....6....12^5 ....40....
```
```
...
MK/WK:TDES:DAT em modo CBC com “IV” ( Initialization Vector ), usando a chave de índice “07”.
```
###### SPE 
...
29 4C BC CF EA CE 1A DA 9B 89 E2 21 22 D7 5C 39
...
```
```
Cartão presente: Apresenta a mensagem definida por RMC_MSG , alternada com uma mensagem de
“RETIRE O CARTÃO”, permanecendo neste estado até a retirada do cartão.
...
```
```
Os dados podem ser divididos em vários blocos para se respeitar a estrutura padrão dos pacotes de
protocolo, conforme descrito na seção 3.1.3.1.
```
#####  Comando
...
0F 0B AF 09 7F 18 61 13 51 10 D1 BF 80 BA 60 C9
...
C1 5D D1 5B A0 5B B0 B6 60 74 A5 E7 CA FA 55 D0
...
C1 CB DF 78 7F D3 DD C6 6C AB 6F A7 B7 97 1E 02
...
C7 26 7D 27 2F 3F 5E F8 78 FC 49 D6 93 99 A7 C5
...
```
```
Se não houver arquivos carregados, o comando é bem-sucedido e a lista retornada é vazia. Não há
ordem específica para a montagem da lista, dependendo exclusivamente das características de
implementação pinpad.
```
#####  Comando
...
```
```
SPE  44 4D 46 30 32 34 00 1E 00 08 54 45 53 54 45 43 48 4F 00 1E 00 08 4D 4F 56 4E 50 49 43 54 DMF024....TESTECHO....MOVNPICT
```
```
A operação é bem-sucedida.
```
...
```
Este comando apresenta no display um arquivo multimídia previamente
carregado no pinpad. O conteúdo será centralizado no display caso suas
dimensões sejam inferiores à capacidade do equipamento.
...
```
```
Os pinpads não são obrigados a suportar todos os formatos de arquivo multimídia previstos por
esta especificação. O SPE deve obter a informação dos formatos suportados através do
comando “GIX” (parâmetro PP_MFSUP).
```
#####  Comando
...
```
```
Este comando é obsoleto. Para esta funcionalidade, o SPE deve usar o comando “GIX” com
PP_TABVERnn.
```
#####  Comando
...
 **PP** 47 54 53 30 30 30 30 31 30 58 30 30 33 45 4D 56 53 54 30 GTS000010XEMVST0 003
...
```
```
SPE  54 4C 49 30 31 32 30 38 30 30 54 41 42 56 45 52 30 30 TLI01200TABVER00 08
```
```
...
definitiva, substituindo as Tabelas EMV anteriores (se existentes). Nesse momento, TLI_TABVER
passa a vigorar para as novas tabelas.
...
```
```
Ao ser acionado, o pinpad mostra uma mensagem no display solicitando a apresentação de um
cartão. Caso seja utilizado um cartão com chip (ICC ou CTLS), o processamento EMV é iniciado
automaticamente. Para isso, o pinpad necessita que as Tabelas EMV estejam carregadas em sua
memória (ver Capítulo 4 ).
```
#####  Comando
...
ST_CARDBLOCKED ...... ICC ou CTLS está bloqueado.
ST_CARDPROBLEMS ... ICC inválido ou com problemas.
ST_CARDINVDATA ..... ICC com dados inválidos ou faltando.
...
no processamento do cartão com _chip_. Este campo volta zerado
para **GCR_CARDTYPE** = “00” (magnético).

**GCR_ACQIDX** N2 Retorna o valor de **TAB_ACQ** do registro da Tabela de AID usada no
processamento do cartão com _chip._ Este campo volta zerado para
**GCR_CARDTYPE** = “00” (magnético).

**GCR_RECIDX** A2 Retorna o valor de **TAB_RECIDX** do registro da Tabela de AID usada
no processamento do cartão com _chip._ Este campo volta zerado
para **GCR_CARDTYPE** = “00” (magnético).
...
```
Este campo não é devolvido para GCR_CARDTYPE = “00”
(magnético) ou “0 5 ” (CTLS simulando tarja).
```
**GCR_PANSEQNO** N2 (^) _Application PAN Sequence Number._
Este campo volta zerado para **GCR_CARDTYPE** = “00” (magnético)
ou “0 5 ” (CTLS simulando tarja).
**GCR_APPLABEL** A16 Etiqueta da aplicação sendo processada, com espaços à direita.
Este campo não é devolvido para **GCR_CARDTYPE** = “00”
(magnético).
**GCR_SRVCODE** N3 (^) _Service Code._
Este campo volta zerado para **GCR_CARDTYPE** = “00” (magnético)
ou “0 5 ” (CTLS simulando tarja).
...
Este campo somente retorna se **GCR_CARDTYPE** = “0 3 ” (ICC EMV).
...
Este campo volta zerado para **GCR_CARDTYPE** = “00” (magnético)
ou “0 5 ” (CTLS simulando tarja), podendo voltar zerado também
para “0 6 ” (CTLS EMV), dependendo da situação.
...
trilha 2 pode atingir 40 caracteres (ver explicação na **seção 5.3** ).
****** Neste caso ele deixa de ter formato “N3” e passa a ter formato “A3”!!
**Se GCR_ACQIDX = “01”:
Id. do Campo Formato Descrição
GCR_ACQRD** N2 (^) Quantidade de bytes representativos no _Application Identifier_
(tamanho ÷ 2).
H32 (^) _DF Name_ ( _tag_ 84h), com FFh à direita.
...
```
- Quando o comando retornar os erros ST_CTLSPROBLEMS, ST_CARDBLOCKED,
    ST_CTLSAPPNAV, ST_CTLSAPPNAUT, ST_CTLSINVALIDAT ou ST_CTLSIFCHG; ou
- Quando o comando retornar o erro ST_CTLSCOMMERR pela segunda vez consecutiva.

#####  Observação #3

```
Se o comando “GCR” retornar ST_CTLSEXTCVM, o SPE deve apresentar uma mensagem ao
...
ST_CARDBLOCKED,ST_CTLSIFCHG ou
...
comandos “GOC” e “FNC”. Estes parâmetros podem coincidir com os existentes no registro da Tabela
de AID (ver seção 4.1.1 ) correspondente à aplicação selecionada no cartão com chip EMV. Neste
caso, os valores não são alterados nas tabelas, sendo somente relevantes para o cartão em
processamento.
...
```
```
Caso “GCR” tenha reportado a passagem de um cartão magnético (ou CTLS simulando tarja), este
...
**“0”** = PAN não consta na Lista de Exceção.
**“ 1 ”** = PAN consta na Lista de Exceção.
...
Se **GOC_METHOD** = “3”, este campo é ignorado pelo pinpad.
...
**Se GCR_ACQIDX** = **“02”:
Id. do Campo Formato Descrição
GOC_ACQPR** S 32 Mensagem a ser apresentada na captura de PIN, seja _online_ ou
_offline_ , já formatada para 2 linhas e 16 colunas.
...
de DUKPT ( GOC_METHOD = “3”). Para MK/WK, este campo é
devolvido zerado.
```
```
...
31 31 38 32 39 46 32 37 39 46 32 36 39 46 33 36
...
chamado apenas para se manter o mesmo fluxo operacional de uma transação online.
```
#####  Comando
...
ou “ cashback ” (se SPE_CASHBACK presente e diferente de zero).
```
```
...
Este campo só é retornado se **PP_CARDTYPE** = “0 3 ” (ICC EMV).
...
Este campo não é devolvido pelo pinpad se **PP_CARDTYPE** = “00”
(magnético) ou “05” (CTLS simulando tarja).
Dependendo da situação, também pode não ser devolvido se
**PP_CARDTYPE** = “06” (CTLS EMV).
...
“05” ou “06”):
“00” = Cartão;
“01”, “07” , “08”, “09”, “14”, “16”, “21” = Telefone móvel;
“02” = Chaveiro;
“03”, “18”, “19”, “23” = Relógio;
“04”, “27” = Etiqueta móvel (“ mobile tag ”);
“05” = Pulseira / bracelete;
“06” = Capa de telefone móvel (“ case/sleeve ”);
“10”, “11”, “12”, “13”, “15”, “17”, “22” = Tablet ou e-Reader ;
Outros valores = Uso futuro.
...
▪ Valor R$ 1128,00, com _cashback_ de R$ 128,00;
...
14 00 0C 30 30 30 30 30 30 30 31 32 38 30 30 00
...
EMV ( PP_CARDTYPE = “03”), CTLS simulando tarja ( PP_CARDTYPE = “0 5 ”) ou CTLS EMV
( PP_CARDTYPE = “06”).
```
#####  Comando
...
comando “GCX” tenha sido executado previamente com sucesso para um ICC
EMV ( PP_CARDTYPE = “03”) ou CTLS EMV ( PP_CARDTYPE = “06”), conforme apresentado no fluxo da
seção 3.7.5.
...
```
```
SPE_ACQREF M Identificador da Rede Credenciadora ( TAB_ACQ ) cujas Tabelas
EMV serão usadas no processamento. Deve-se selecionar,
obrigatoriamente, uma das Redes Credenciadoras devolvidas em
PP_AIDTABINFO na resposta de “ GCX ”.
```
```
SPE_TRNTYPE O Tipo de transação a ser efetuada:
00h = Compra;
...
“x **1** xxx” = Transação não pode ser aprovada _offline_ (só usado se
ICC EMV).
“xx **1** xx” = Não permite _bypass_ de PIN.
“xxx00” = RUF.
Se este parâmetro estiver ausente, o pinpad considerará este
...
ST_CARDINVDATA ...... ICC com dados inválidos ou faltando.
ST_ERRFALLBACK ....... Erro sujeito a “fallback” para tarja.
ST_TIMEOUT .............. Esgotado tempo definido em
SPE_TIMEOUT para captura de PIN.
```
```
...
“GCX” tenha requerido aprovação online ( PP_GOXRES = “ 2 xxxx”), conforme apresentado na seção
3.7.5.
...
requerido.
(^2) Neste caso, o SPE pode cancelar a operação mediante envio de um **«CAN»** , porém deve considerar o resultado da
transação (aprovação/negação) informado pela resposta da Rede Credenciadora, dado que o pinpad somente
devolverá um **«EOT»** e, portanto, o campo **PP_FCXRES** não estará presente.
 **Obsoleto**
 **Blocante**
 **ABECS**
...
O comando pode ser utilizado para executar um “power on” , “reset” ou “power off” no ICC, ou
apenas para consultar o status atual do leitor e do cartão.
```
```
...
devolvidos em G0302_EMVDAT. As tags devem ser simplesmente
concatenadas, respeitando-se sua regra de formação (ver seção
7.1 ).
```
#####  Resposta
...
```
```
G0302_EMVDAT H..512 Dados EMV, no formato TLV (ver seção 7.1 ). O pinpad concatena
os dados pedidos por G0302_TAGS , se encontrados, respeitando a
ordem em que foram solicitados.
```
```
 Obsoleto
...
são transferidas para o pinpad utilizando-se os comandos descritos na seção 3.5.
```

```
A consistência dos registros enviados é de total responsabilidade do SPE, sendo que o pinpad
não faz críticas complexas, como, por exemplo, identificar colisões de índices e registros. O
pinpad simplesmente despreza registros cujo conteúdo é notavelmente inválido.
```
### 4.1. Tipos de Tabela
...
**T1_FLRLIMIT** H8 9F1Bh (^) _Terminal Floor Limit_ (valor “default” para uso em
“ **GCR** ” ou “ **GCX** ”), em centavos, expresso na moeda
...
“7” = Não suporta (reservado); Suporta Pure
Contactless;
“8” ou “9” = Suporta Discover D-PAS EMV Mode;
“A” = Suporta JCB Contactless (uso futuro)
“B” = Suporta Union Pay Quick Pass (uso futuro);
“C” = Suporta Interac Flash (uso futuro); e
“D” = Suporta kernel EMVCo C-8 (uso futuro).
“0” ou outro valor = Não suporta.
...
Id. do Campo Formato Tag Descrição
```
**T1_CTLSTACDEF** H10(B5) DF8120h (^) _Terminal Action Code – Default_ para CTLS. Se **TAB_LEN**
< 314, o pinpad assume o valor de **T1_TACDEF**.
**T1_CTLSTACDEN** H10(B5) DF8121h (^) _Terminal Action Code – Denial_ para CTLS. Se **TAB_LEN**
< 314, o pinpad assume o valor de **T1_TACDEN**.
**T1_CTLSTACONL** H10(B5) DF8122h (^) _Terminal Action Code – Online_ para CTLS. Se **TAB_LEN**
< 314, o pinpad assume o valor de **T1_TACONL**.
**T1_CTLSTRMCP** H6(B3) 9F33h (^) _Terminal Capabilities_ para CTLS. Se **TAB_LEN** < 3 40 , o
pinpad assume o valor de **T1_TRMCAPAB** , desativando
os bits “ _Plaintext PIN for ICC verification_ ” (Byte 2 b8)
e “ _Enciphered PIN for offline verification_ ” (Byte 2 b5).
**T1_MOBCVM
T1_RUF2**
N1 Suporte à verificação de portador no próprio
dispositivo usado para efetuar a transação (telefone
celular, por exemplo).
“1” = Sim / “0” = Não
Se **TAB_LEN** < 340, o pinpad assume o valor “0”.
RUF (valor ignorado pelo pinpad).
**T1_CTLSADDTC** H10(B5) 9F40h (^) _Additional Terminal Capabilities_ para CTLS.
Se **TAB_LEN** < 3 40 , o pinpad assume o valor de
**T1_ADDTRMCP**.
...
“1” = Sim / “0” = Não
Se **TAB_LEN** < 3 40 , o pinpad assume o valor “0” (não).
...
“0” = _Check Sum_ ausente (não recomendado); ou
“1” = _Check Sum_ presente.
**T2_CHECKSUM** H40 (^) _Certification Authority Public Key Check Sum_ , ignorado
se **T2_CHKSTAT** = “0”.
...
os comandos “GTS” ou “ GIX ” (com PP_TABVERnn ).
...
“injetadas” pelo fabricante, considerando-se dois algoritmos diferentes:
```
- **MK/WK TDES** ; e
- **DUKPT TDES**.
...
Desta forma, esta especificação considera o seguinte mapeamento de chaves, diferenciando quatro
tipos para cada índice numérico existente:
...
- O índice “00” é válido e, considerando-se que o máximo índice permitido é “99”, pode-se ter até
    ...
    modelo de pinpad, respeitando-se o mínimo definido na **seção 6.1.2** (por exemplo, um
    determinado modelo de pinpad pode permitir até 77 chaves DUKPT:TDES, de índices “00” a “ 76 ”).
- As chaves DUKPT:TDES de dados (“DAT”) permitem diferentes variantes no momento do uso (ver
    **seção 5.1.1** ), entretanto a existência destas variantes não requer nenhum tratamento especial no
...
obtendo assim a chave KSEC aleatória gerada pelo pinpad.
```
...
- O pinpad detecta alguma inconsistência no comando criptografado, retornando “ **ERR009** ”
...
```
Se uma Master Key (TDES) do pinpad for conhecida, a WKPAN poderá ser gerada externamente
pelo SPE e fornecida ao pinpad criptografada por esta Master Key. O tamanho da chave WKPAN
deverá respeitar o tamanho da Master Key conhecida (16 bytes para TDES).
```
**SPE Pinpad**
...
```
Mesmo estando habilitada a modalidade “PAN Criptografado”, em algumas situações (como
explicado na seção 5.3.1 ) uma ou mais trilhas retornadas pelo pinpad podem não sofrer criptografia
alguma, no caso em que não tenha sido possível isolar um PAN válido. Entretanto, esta especificação
não prevê uma forma de informar ao SPE esta ocorrência, podendo gerar erros quando este tentar
...
- O pinpad preserva “em claro” os caracteres iniciais da trilha 1, de acordo com a quantidade
    solicitada em **SPE_OPNDIG** , desconsiderando-se o caractere de formato (normalmente “ **B** ”), que
    é sempre devolvido em claro.
...
```
O diagrama a seguir ilustra a criptografia de uma trilha de 39 caracteres (“ B9994444333322221111^
NOME^151260123487 9 ”) usando-se criptografia de bloco tipo ECB , em que se deseja preservar “em
claro” os 8 primeiros caracteres:
```
```
B 9 9 9 4 4 4 4 3???????????????????????????????
```
...
```
O diagrama a seguir ilustra a criptografia de uma trilha de 37 posições (“ 66733246732413=
1512601234879534275432 ”) usando-se criptografia de bloco tipo CBC , sem “IV” ( Initialization
Vector ), em que se deseja preservar “em claro” os 4 primeiros dígitos:
```
```
...
- Não há uma quantidade máxima de registros de Tabelas de AID ou CAPK por rede credenciadora.
    ...
    acima).
- No mínimo 64 chaves de criptografia (índices **“0 0 ”** a **“ 63 ”** ) para os seguintes tipos:
...
```
A norma EMV para ICC ( EMV#3 ) prevê que o comando GENERATE APPLICATION CRYPTOGRAM (ou
“GENERATE AC”) seja enviado ao cartão até duas vezes durante o processamento de uma transação.
Caso o SPE envie comandos ao pinpad que resultem na submissão de mais de dois GENERATE AC ao
cartão em uma única transação, isso deve ser caracterizado como tentativa de ataque de replay.
Para identificar essa situação, o pinpad deve contar quantos GENERATE AC estão sendo enviados ao
cartão durante uma transação e retornar o erro ST_SECURITY como resposta ao comando que faria
terceira tentativa.
```
```
Para este fim, define-se como uma transação todo o período em que um ICC está inserido no
pinpad, independentemente de quantas vezes o chip foi ativado ( reset ) ou quais comandos
foram a ele submetidos. Uma nova transação somente será iniciada quando o cartão for
removido e inserido novamente.
```
```
De acordo com esta especificação, o comando GENERATE AC somente é enviado ao ICC nos
comandos “ GOC ”, “ GOX ”, “ FNC ”, “ FCX ” ou de forma explícita no comando “ CHP ” (se o byte INS
dentro de CHP_CMD tiver o valor “AE”).
O fluxograma a seguir ilustra uma maneira simples de se implementar essa proteção, valendo-se de
uma variável “CtrGAC” para contar quantos GENERATE AC foram submetidos dentro da mesma
transação:
```
```
 ST_SECURITY
...
Nenhum outro comando deve influenciar o valor do “CtrGAC”, sendo que este contador é
mantido inalterado mesmo na execução dos comandos “ **OPN** ”, “ **CLO** ” ou “ **CLX** ”.
...
```
```
Caso o pinpad já esteja operando em “Comunicação Segura” ou “PAN Criptografado” e receba
um novo “OPN” (seja este clássico ou seguro) sem ter sido precedido por “CLO/CLX” , esses
recursos de segurança devem ser automaticamente desativados, como se um “CLO/CLX”
tivesse de fato sido recebido. A “Comunicação Segura” será restabelecida caso seja solicitada
no novo “OPN” recebido.
```
```
...
disso, deve:
```
- Desativar ou esmaecer o _“backlight”_ do _display_ (se existente).
...
```
```
ST_NOCARD Não há cartão presente no acoplador ou antena. Se a antena CTLS estiver
ativa, esta deve ser desativada.
```
```
ST_DUMBCARD ICC inserido, mas não responde (“mudo”).
```
```
ST_ERRCARD Erro de comunicação entre o pinpad e o cartão.
```
```
ST_CTLSMULTIPLE Mais de um CTLS foi apresentado ao leitor simultaneamente (a antena
deve ser desativada).
```
```
ST_CTLSCOMMERR Erro de comunicação entre o pinpad (antena) e o CTLS (a antena deve ser
desativada).
```
```
ST_INVPARM Referência fornecida em CHP_SLOT não é suportada pelo equipamento
(slot de SAM inexistente ou CTLS não suportado).
```
##### 6.5.2.3. Troca de comando (CHP_OPER = “2”)
...
```
```
ST_ERRCARD Erro de comunicação entre o pinpad e o ICC, ou cartão não foi ativado
...
```
```
ST_SECURITY Tentativa de ataque de replay (ver seção 6.3.5 ).
```
...
recebida, posicionando-a no canto superior esquerdo de acordo com o alinhamento definido em
DEX_OPTIONS.
...
```
```
Para o tratamento “tipo de mensagem” em DEX_OPTIONS , deve-se utilizar recursos de cor e/ou
imagens para melhor indicar a intenção do SPE. Por exemplo, para uma mensagem de erro, pode-se
apresentar o texto em branco sobre um fundo vermelho, com um ícone “”.
```
```
A tabela a seguir indica o som a ser emitido pelo pinpad em cada situação:
```
```
DEX_OPTIONS Som a ser emitido
```
```
“xx0xxx” = Genérica (default) Nenhum.
```
```
“xx1xxx” = Sucesso Um ou dois tons mais agudos de curta duração.
```
```
“xx2xxx” = Erro Tom mais grave de longa duração.
```
...
```
```
“xx3xxx” = Alerta Tom mais grave de curta duração.
```
```
“xx4xxx” = Informativa Nenhum.
```
#####  Situações de exceção:
...
▪ SPE_MTHDDAT = “1x” e SPE_WKENC não foi recebido.
```
...
- No caso de captura de dados alfanuméricos ( **SPE_GCDOPT** = “1xxx”), a forma como isso é feito
    depende dos recursos do pinpad e não é objeto desta especificação.
...
estabelecidos pela ANSI X9.8 e X9.24.
```
- Este comando usa somente chave de PIN.
...
não foi fornecido.
▪ SPE_MTHDDAT = “1x” e SPE_WKENC não foi fornecido.
...
- O pinpad não deve retornar erro caso o título informado em **SPE_DSPMSG** seja grande a ponto de
    não deixar espaço no _display_ para as opções do menu. Neste caso, esse título deve ser truncado
    para que seja possível apresentar ao menos uma opção para seleção.
...
A mensagem de resposta do protocolo comporta mais de 160 nomes de arquivo. Na eventualidade
de esta quantidade ser ultrapassada, o pinpad deve retornar sucesso e ignorar os arquivos cujos
nomes não couberem na resposta.
```
#####  Situações de exceção:
...
```
```
GCR_AMOUNT = 0 (zero) GCR_AMOUNT ≠ 0 (zero)
```
```
CTLS não suportado INSIRA OU PASSE O CARTÃO
```
...
```
```
Caso o número de “registros candidatos” supere a capacidade definida por esta especificação
...
```
```
ST_CARDINVALIDAT ▪ A única aplicação compatível no cartão está invalidada (SELECT retornou
SW1/SW2 = 6283h).
```
```
...
Union Pay), conforme descrito em  **VCPS** ,  **PPMChip** ,  **ExpPay,**  **D-PAS ou**  **QckPass**.
...
```
```
1 Offline Data Authentication for
...
```
```
6 (Contact Chip) Offline PIN
supported
```
```
1 se o bit 8 ou o bit 5 do 2º byte de
T1_TRMCAPAB estiverem ativos.
```
```
5 a 1 RFU 00000 (fixo)
```
...
```
```
6 a 1 RFU 000000 (fixo)
```
```
4 8 a 1 RFU 00000000 (fixo)
```
...
```
```
5 Relay resistance protocol (RRP)
supported
```
```
1 (fixo)
...
6 _Signature (paper) (Contact)_ Bit 6 do 2º byte de **T1_TRMCAPAB**
5 _Enciphered PIN verification
performed by ICC (Contact)_
Bit 5 do 2º byte de **T1_TRMCAPAB**
4 _No CVM required (Contact)_ Bit 4 do 2º byte de **T1_TRMCAPAB**
2 _Plaintext PIN verification
performed by ICC (Contact)_
Bit 8 do 2º byte de **T1_TRMCAPAB**
3 8 _Mag-Stripe mode contactless
transactions not supported_
1 (fixo)
7 _EMV mode contactless transactions
not supported_
0 (fixo)
...
```
```
1 8 Mag stripe mode supported 0 (fixo)
```
```
7 RFU 0 (fixo)
```
...
```
```
6 Consumer Device CVM for Transit
MCC
```
```
0 (fixo)
...
#####  Parâmetros específicos – Union Pay Quick Pass

```
Se Union Pay Quick Pass ( T1_CTLSMODE = “ B ”), os seguintes parâmetros adicionais devem ser
fornecidos ao Kernel EMV CTLS, de acordo com  QckPass :
```
```
Objeto Tag Valor
```
...
```
```
7 Full transaction flow in Contactless
interface Support
```
```
0 (fixo)
...
```
```
5 Full transaction flow in contact
interface Support
```
```
Bit 6 do 1º byte de T1_TRMCAPAB
```
```
4 Offline-only terminal Ativar se T1_TRMTYP = “x3” ou “x6”
```
```
3 Online PIN Supported Bit 7 do 2º byte de T1_CTLSTRMCP
```
...
```
```
6 a 1 RFU 00000 0 (fixo)
```
```
4 8 fDDA v1.0 Supported 1 (fixo)
```
```
7 a 1 RFU 0000000 (fixo)
```
...
```
Caso o CTLS seja processado com sucesso na modalidade PayPass Mag Stripe , Expresspay Magstripe
Mode ou D-PAS MS Mode magstripe (simulação de tarja magnética), a resposta ao comando “ GCR ”
deve devolver os seguintes campos:
```
```
Id. do Campo Dado retornado
```
...
```
Caso o CTLS seja processado com sucesso na modalidade qVSDC , PayPass M/Chip , Expresspay EMV
Mode , D-PAS EMV Mode EMV full (não simulando tarja magnética) a resposta ao comando “ GCR ”
deve devolver os seguintes campos:
```
```
Id. do Campo Dado retornado
...
```
```
ST_INVPARM O formato do comando ou o conteúdo dos parâmetros recebidos não estão
condizentes com esta especificação.
```
```
...
( Outcome = “Try Again” ) e T1_MOBCVM ≠ “1”.^3
```
```
ST_CARDBLOCKED Comando SELECT retorna erro SW1/SW2=6A81h (cartão foi bloqueado).
```
...
```
(^3) Isto é feito para se manter compatibilidade com um SPE antigo que não conhece ST_CTLSEXTCVM.
...
( _Outcome = “Try Again”_ ) e **T1_MOBCVM** = “1”.

ST_CTLSIFCHG ▪ Kernel CTLS solicitou “mudança de interface” para processamento usando
ICC ou cartão magnético ( _Outcome = “Try Another Interface”_ ).
▪ Se **GCR_AMOUNT** ≥ **T1_CTLSTRNLIM** e cartão Discover D-PAS ou Visa
payWave.

 De acordo com  **EMV#CtlsB** seções **3.3.2.6** até **3.3.3.5** , qualquer erro no comando SELECT faz
com que o AID em questão seja retirado da lista de candidatos e, se a lista estiver vazia, deve-se
solicitar mudança de interface. Assim sendo, estas situações acabam acarretando
 **ST_CTLSIFCHG**.
...
Se **GCR_ACQIDX** = **“01”** e **GOC_ACQPR** = “xx **1** ”, deve-se impedir o _bypass_ do PIN, seja este online ou
offline.
...
```
Esta etapa do processamento EMV sempre será efetuada, independentemente do valor de
GOC_RISKMAN (obsoleto).
...
Essa implementação não pode usar o bit “ Merchant Forced Transaction Online ” do TVR para
impedir a aprovação offline, pois isso não é aceito nas certificações de bandeiras.
```
...
ST_SECURITY Tentativa de ataque de _replay_ (ver **seção 6.3.5** ).
...
```
```
Caso a verificação de portador requisite PIN Online, o bypass não deve ser permitido para
cartões CTLS (a tecla [OK/ENTRA] deve ser ignorada se o campo de entrada estiver vazio).
```
#####  Dados de Saída
...
```
FNC_DECISION Decisão do cartão no comando 2nd GENERATE AC:
“0” → Cartão retornou TC (aprovada).
“1” → Cartão retornou AAC (negada), porém Rede Credenciadora havia
aprovado a transação.
“2” → Cartão retornou AAC (negada), acatando a decisão da Rede
Credenciadora.
```
```
FNC_EMVDTLEN
...
- _Track 2 Discretionary Data_ ( _tag_ 9F20h);
- _Track 1 Data_ ( _tag_ 56h); e
...
(como o PAN, por exemplo), além do _TVR_ , _TSI_ e _CVM Results_. A tabela a seguir lista as _tags_ dos
parâmetros que, caso recebidos, não devem ser fornecidos ao Kernel EMV para uso no
processamento:

```
42h 4Fh 50h 56h 57h 5Ah 5F20h 5F24h 5F25h 5F28h
```
```
5F2Dh 5F30h 5F34h 5F50h 5F53h 5F54h 5F55h 5F56h 61h 6Fh
```
```
70h 73h 77h 80h 82h 84h 87h 88h 8Ch 8Dh
```
```
8Eh 8Fh 90h 92h 93h 94h 95h 97h 9Bh 9Dh
```
```
9F05h 9F07h 9F08h 9F0Bh 9F0Dh 9F0Eh 9F0Fh 9F10h 9F11h 9F12h
```
```
9F13h 9F14h 9F17h 9F1Fh 9F20h 9F23h 9F26h 9F27h 9F2Dh 9F2Eh
```
```
9F2Fh 9F32h 9F34h 9F36h 9F38h 9F3Bh 9F42h 9F43h 9F44h 9F45h
9F46h 9F47h 9F48h 9F49h 9F4Ah 9F4Bh 9F4Ch 9F4Dh 9F4Fh A5h
```
```
BF0Ch
```
...
```
```
Caso existam “registros candidatos” com AIDs conflitantes^4 , o pinpad deverá eliminar os
...
```
```
Caso os “registros candidatos” resultantes contenham AIDs conflitantes^5 (de mesmo valor), o
...
conforme descrito em  VCPS ,  PPMChip ,  ExpPay ,  D-PAS e  QckPass.
```
```
...
“ GOX ”.
```
(^5) Dois AIDs são conflitantes quando forem exatamente iguais ou quando o AID de menor tamanho estiver
inteiramente contido à esquerda do AID de maior tamanho. Por exemplo, os AIDs ‘A0 00 00 00 03 10 10’ e ‘A0 00 00
00 03 10 10 98 76’ são conflitantes.
...
- Se **T1_CTLSMODE** = “ **B** ”, os mesmos parâmetros adicionais definidos na **seção 6.8.1.3**
    ( **Parâmetros específicos - Discover D-PAS** ) devem ser fornecidos ao Kernel EMV CTLS.
...
```
Caso o CTLS seja processado com sucesso na modalidade de PayPass Mag Stripe , Expresspay
Magstripe Mode ou D-PAS MS Mode magstripe (simulação de tarja magnética), a resposta ao
...
podendo ser o Application Label ( tag 50h) ou Application Preferred
Name ( tag 9F12h).
Importante: retornar campo vazio (tamanho zero) se o Application Label
estiver ausente no cartão.
...
```
Caso o CTLS seja processado com sucesso na modalidade qVSDC , PayPass M/Chip , Expresspay EMV
Mode ou D-PAS EMV Mode EMV full (não simulando tarja magnética), a resposta ao comando “ GCX ”
deve devolver os seguintes campos:
```
```
Id. do Campo Dado retornado
...
podendo ser o _Application Label_ ( _tag_ 50h) ou _Application Preferred
Name_ ( _tag_ 9F12h).
Importante: retornar campo vazio (tamanho zero) se o _Application Label_
estiver ausente no cartão.
**PP_ISSCNTRY** (^) _Issuer Country Code_ ( _tag_ 5F28h), se existente no cartão.
**PP_CARDEXP** (^) _Application Expiration Date_ ( _tag_ 5F24h), se existente no cartão.
**PP_EMVDATA** Dados definidos pela _tags_ em **SPE_TAGLIST** , no formato TLV (se
existentes), na mesma ordem em que foram solicitados. Ver restrições na
**seção 6.8.6.3**
Este campo é mandatório sempre que **SPE_TAGLIST** existir no comando,
mesmo que nenhum objeto seja encontrado (caso em que é retornado
...
**PP_DEVTYPE**
(Visa payWave)
De acordo com os bits de 5 a 1 do 1º byte do _Form Factor Indicator_ (tag
9F6Eh):
00000 ( _Standard card_ ) → “00” (Cartão)
00001 ( _Mini-card_ ) → “00” (Cartão)
00011 ( _Consumer mobile phone_ ) → “01” (Telefone móvel)
00100 ( _Wrist-worn device_ ) → “05” (Pulseira)
outro valor → “99” (não definido)
**PP_DEVTYPE**
(Mastercard
PayPass)
_Device Type_ - quinto e sexto bytes de _Third Party Data_ ( _tag_ ‘9F6E’).
...
```
```
Se o bit 8 do 2 º byte de Card Transaction Qualifiers (tag 9F6Ch) estiver
ativo (“ Consumer Device CVM Performed ”), retornar “01” (Telefone
móvel), caso contrário retornar “00” (Cartão).
```
#####  Situações de exceção:
...
ST_CTLSCOMMERR ▪ Erro de comunicação entre o pinpad (antena) e o CTLS.
▪ O Kernel CTLS solicitou a verificação do portador no dispositivo móvel
( Outcome = “Try Again” ) e T1_MOBCVM ≠ “1”.^6
```
```
ST_CARDBLOCKED Comando SELECT retorna erro SW1/SW2=6A81h (cartão foi bloqueado).
```
...
```
ST_CTLSIFCHG ▪ Kernel CTLS solicitou “mudança de interface” para processamento usando
ICC ou cartão magnético ( Outcome = “Try Another Interface” ).
▪ Se GCR_AMOUNT ≥ T1_CTLSTRNLIM e cartão Discover D-PAS ou Visa
payWave.
```
```
...
 ST_CTLSIFCHG.
```
(^6) Isto é feito para se manter compatibilidade com um SPE antigo que não conhece ST_CTLSEXTCVM.
...
Enquanto o pinpad aguarda a digitação de um PIN, o comando pode ser cancelado pelo SPE através
do envio de um byte **«CAN»**.
Se **SPE_GOXOPT** = “xx **1** xx”, deve-se impedir o _bypass_ do PIN, seja este online ou offline.
O pinpad deve finalizar a operação com ST_TIMEOUT se o tempo de inatividade em uma tela de
...
```
```
Se SPE_GOXOPT = “x 1 xxx”, o Kernel EMV nunca poderá sugerir aprovação offline (TC) ao
cartão. Essa implementação não pode usar o bit “ Merchant Forced Transaction Online ” do TVR
para impedir a aprovação offline, pois isso não é aceito nas certificações de bandeiras.
```
#####  Card Action Analysis.
...
```
PP_FCXRES Decisão do cartão no comando 2nd GENERATE AC:
“ 0 xx” → Cartão retornou TC (aprovada);
“ 1 xx” → Cartão retornou AAC (negada).
```
```
PP_EMVDATA Dados definidos pela tags em SPE_TAGLIST , no formato TLV (se
existentes), na mesma ordem em que foram solicitados. Ver restrições na
...
mesmo que ocorra algum erro de processamento, “timeout” ou que a operação seja
interrompida pela tecla [CANCELA].
```
#####  Dados de Saída
...
**T1_TACDEF** Não importa.

**T1_TACDEN** Não importa.

**T1_TACONL** Não importa.
...
Em caso de divergência, adotar “ **9F37 04 00...00** ”.
...
```
```
Gerenciadora [CLEAR/BACKSP] → [OK/ENTRA] → [0] PP_MODEL
PP_MANVERS
PP_SPECVER
```
```
Abecs [CLEAR/BACKSP] → [OK/ENTRA] → [1] PP_APPVERS
PP_SPECVER
PP_KRNLVER
PP_CTLSVER
```
```
Extensão [CLEAR/BACKSP] → [OK/ENTRA] → [9] PP_GENVERS
PP_SPECVER
```
```
...
- **Seção 2.3.4** : Incluída resposta “ **ERR** ” para situações de exceção.
